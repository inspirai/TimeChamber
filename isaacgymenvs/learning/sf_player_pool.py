...
class SFPlayerPool:
    ...
    def inference(self, input_dict, res_dict, processed_obs):
        ...
        # b = time.time()
        # torch.cuda.synchronize(self.players[0].device)
        # print(res_dict)
        # print(b - a)
...
class SFPlayerVectorizedPool(SFPlayerPool):
    ...
    def inference(self, input_dict, res_dict, processed_obs):
        ...
        for i, player in enumerate(self.players):
            if len(player.env_indices) == 0:
                continue
            # if player.model and i == 0:
            #     input_dict['obs'] = processed_obs[player.env_indices]
            #     print(self.vectorized_model.value_mean_std.training)
            #     print(player.model(input_dict)['values'][0])
            #     print(out_dict['values'][i][player.env_indices][0])
            for key in res_dict:
